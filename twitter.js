 ...
 * @param callback
 */
Twitter.prototype.uploadVideo = function (params, isExternal, accessToken, accessTokenSecret, callback) {
  if (isExternal) {
    this.uploadExternalMediaChunked(params, "video/mp4", accessToken, accessTokenSecret, callback);
  } else {
...
Twitter.prototype._appendMediaUpload = function(oauth, data, mediaId, segmentIndex, cb) {
  const options = {
    url: uploadBaseUrl + 'media/upload.json',
    oauth,
    formData: {
...
Twitter.prototype._finalizeMediaUpload = function(oAuthCredentials, mediaId, cb) {
  const options = {
    url: uploadBaseUrl + 'media/upload.json',
...
Twitter.prototype._getStatusMediaUpload = function(oa, oAuthCredentials, mediaId, lastProgressPercent, cb) {
  var url = uploadBaseUrl + 'media/upload.json';
  var params = {
    command: 'STATUS',
    'media_id': mediaId
  }
...
Twitter.prototype.uploadExternalMediaChunked = function (params, media_type, accessToken, accessTokenSecret, callback) {
  var oauth = {
    consumer_key: this.consumerKey,
    consumer_secret: this.consumerSecret,
    token: accessToken,
  ...
  var _streamMediaToTwitter = this._streamMediaToTwitter;
  var _getStatusMediaUpload = this._getStatusMediaUpload;
  ...
  this._initMediaUpload(oauth, media_type, params.size, function (error, body) {
    ...
    _streamMediaToTwitter(oauth, params.media, mediaId, _appendMediaUpload, _finalizeMediaUpload, function (error, mediaId) {
      if (error) {
        return callback(error);
      }
      let state = 'pending';
